...
def entry(bot, update):
    try:
        ...
        pass
    if update.message and update.message.chat.id == -1001440317798:
        if update.message.reply_to_message and update.message.reply_to_message.forward_from_chat and update.message.reply_to_message.forward_from_chat.id == -1001375778275:
            try:
    ...
    if update.message and update.message.text and update.message.chat.type == "private":
        ...
        if any(x in update.message.text for x in matches):
            ...
            if not temp_flood_ban_list.get(update.message.chat.id):
                res = bot.sendMessage(chat_id=-1001375778275, text=update.message.text+"\n\n"+"from: "+update.message.chat.first_name+" #q"+str(update.message.chat.id))
